...
import (
	...
	"github.com/dops-cli/dops/progressbar/cwriter"
...
func (p *Progress) Log(message string) *Progress {
	...
	p.Add(0, MultiBarLog(message), TrimSpace()).SetTotal(0, true)
	return p
}
...
func (p *Progress) NewLine() *Progress {
	...
	bar := p.Add(0, MultiBarLog(""), TrimSpace())
	bar.SetTotal(0, true)
	return p
}
...
func (p *Progress) LogWithPriority(message string, priority int) *Progress {
	...
	bar := p.Add(0, MultiBarLog(message), TrimSpace())
	bar.SetPriority(priority)
	bar.SetTotal(0, true)
	return p
}
