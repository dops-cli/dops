...
func TestActionFunc(_ *cli.Context) error {
	for _, pkg := range packages {
		var packageName string

		if pkg == "cli" {
			packageName = "github.com/dops-cli/dops/cli"
		} else {
			packageName = fmt.Sprintf("github.com/dops-cli/dops/cli/%s", pkg)
		}
...
func checkBinarySizeActionFunc(_ *cli.Context) (err error) {
	const (
	...
	if isMoreThanDesiredMax {
		...
		fmt.Printf("  %s %s is the target max size", checksPassedEmoji, desiredMaxSizeString)
	}
